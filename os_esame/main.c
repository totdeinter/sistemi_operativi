...
/*typedef struct{
    int* c_ass; //array di interi con le occorrenze di ogni carattere trovate nel file i-esimo
...
int main(int argc, char **argv){
    ...
    //array di array che contiene tutte le strutture ricevute dai figli
    ...
    for(int i = 0; i < N; i++){
        ...
        if(n != L * sizeof(int)){ //errore nella lettura
            ...
            memmove(arr[i], arr_letto_da_pipe, L * sizeof(int));
        }
        ...
        }else if(!WIFEXITED(status) || WEXITSTATUS(status) != EXIT_SUCCESS || err[i] != 0){
    ...
    for(int i = 0; i < N; i++){
        if(err[i] != 0){//errore nel figlio i-esimo
            printf("%s %d Error\n", argv[i + 2], pidFigli[i]);
            errore=1; //segnaliamo c'e' stato un errore nel programma
            //break;
        }else{
            printf("%s %d ", argv[i + 2], pidFigli[i]);
            for(int j = 0; j < L; j++){ //stampo i caratteri
                if(j == L-1) printf("%c: %d", stringa[j], arr[i][j]);
                else printf("%c: %d, ", stringa[j], arr[i][j]);
            }
            printf("\n");
        }
    }
